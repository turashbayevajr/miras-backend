...
export class CourseProgressService {
  ...
  try {
    const enrollments = await this.enrollmentService.getAllEnrollmentsWithCourseAndLessons();
    ...
    for (const enrollment of enrollments) {
      ...
      const submissions = await this.submissionService.getPassedByUser(enrollment.userId);
      ...
      for (const lesson of lessons) {
        ...
        const homeworkSubmission = lesson.homework
          ? submissions.find(s => s.homeworkId === lesson.homework?.id && s.passed)
          : null;
      ...
      const progressPercent = Number(((completedLessons / totalLessons) * 100).toFixed(2));
      const averageScore = usedScores.length
        ? Number((usedScores.reduce((a, b) => a + b, 0) / usedScores.length).toFixed(2))
        : null;
