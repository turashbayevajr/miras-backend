export default {
  ...
  NOT_FOUND_BY_ID: (entity: string, id: string | number) => `${entity} with ID '${id}' not found.`,
  INVALID_RELATION: (entity: string, value: any) => `Referenced ${entity} (${value}) does not exist.`,

  DATABASE_CREATE_ERROR: (entity: string) => `Failed to create ${entity}.`,
  DATABASE_FETCH_ERROR: (entity: string) => `Failed to fetch ${entity}.`,
  DATABASE_FETCH_ERROR_BY_ID: (entity: string, id: string | number) => `Failed to fetch ${entity} with ID '${id}'.`,
  DATABASE_UPDATE_ERROR: (entity: string, id: string | number) => `Failed to update ${entity} with ID '${id}'.`,
  DATABASE_DELETE_ERROR: (entity: string, id: string | number) => `Failed to delete ${entity} with ID '${id}'.`,
